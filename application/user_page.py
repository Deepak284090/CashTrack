...
class UserPage:
    ...
    def show_user_home(self):
        ...
        ttk.Button(nav_frame, text="Add Expense", command=self.add_expense, bootstyle=PRIMARY).pack(side="left", padx=10, pady=5)
        ttk.Button(nav_frame, text="Add Earning", command=self.add_earning, bootstyle=SUCCESS).pack(side="left", padx=10, pady=5)
        ttk.Button(nav_frame, text="Analyze Expense", command=self.analyze_expense, bootstyle=INFO).pack(side="left", padx=10, pady=5)
        ttk.Button(nav_frame, text="Set Expense Limit", command=self.set_expense_limit, bootstyle=WARNING).pack(side="left", padx=10, pady=5)
        ttk.Button(nav_frame, text="Logout", command=self.on_logout, bootstyle=DANGER).pack(side="right", padx=10, pady=5)
    ...
    def add_earning(self):
        ...
        def save_earning():
            try:
                ...
                ttk.Label(self.right_frame, text="Earning Added!", bootstyle="success").pack()
        ...
        ttk.Button(self.right_frame, text="Save Earning", command=save_earning, bootstyle=SUCCESS).pack()
    ...
    def add_expense(self):
        ...
        def save_expense():
            try:
                ...
                if expense_limit is not None and amount > expense_limit:
                    messagebox.showwarning("Expense Limit Exceeded", f"You are exceeding your expense limit of ${expense_limit:.2f}.")

                SpendingsManager.add_spending(self.username, amount, description, date, category)
                ttk.Label(self.right_frame, text="Expense Added!", bootstyle="danger").pack()
        ...
        ttk.Button(self.right_frame, text="Save Expense", command=save_expense, bootstyle=DANGER).pack()

    def analyze_expense(self):
        self.clear_content(self.right_frame)

        ttk.Label(self.right_frame, text="Analyze Expenses", font=("Arial", 12, "bold")).pack(pady=5)

        period_options = ["Monthly", "Quarterly", "Annually"]
        period_var = ttk.StringVar(value="Monthly")
        period_dropdown = ttk.Combobox(self.right_frame, textvariable=period_var, values=period_options, state="readonly")
    ...
    def set_expense_limit(self):
        ...
        def save_limit():
            try:
                ...
                ttk.Label(self.right_frame, text=f"Expense limit set to: ${limit:.2f}", bootstyle="success").pack()
